...
public class SDPFactoryTest {
	...
	public void testRPRO6991() {
		...
		for (MediaField media : medias) {
			...
			if (SDPMediaType.video.equals(mediaType)) {
				...
				List<AttributeField> videoCodecs = media.getAttributeSelections(AttributeKey.rtpmap,
				...
				if (selectedVideoCodec == RTPCodecEnum.H264_PMODE1) {
					...
					if (isOpera && profileSent != null) {
						...
						if (profileSent.startsWith("42")
								&& (profileSent.endsWith("1f") || profileSent.endsWith("1e"))) {
					...
					// pmode-1 only here
					fmtp = new AttributeField(AttributeKey.fmtp,
							String.format("%d profile-level-id=%s;level-asymmetry-allowed=1;packetization-mode=1",
									videoPayloadType, profile));
				}
				...
				if (selectedVideoCodec.payloadType != videoPayloadType) {
					rtpmap = rtpmap.replace(String.valueOf(selectedVideoCodec.payloadType),
							String.valueOf(videoPayloadType));
				}
		...
		if (audio != null) {
			...
			for (RTPCodecEnum codec : RTPCodecEnum.values()) {
				if (codec.encodingName.equals(audioCodecName)) {
					...
					audio.addAttributeField(
							new AttributeField(AttributeKey.rtpmap, RTPCodecEnum.getRTPMapString(selectedAudioCodec)));
					if (localSdp.isFirefox()) {
						audio.addAttributeField(new AttributeField(AttributeKey.mid, "sdparta_0"));
						audio.addAttributeField(new AttributeField(AttributeKey.extmap,
		...
		if (video != null) {
			...
			for (RTPCodecEnum codec : RTPCodecEnum.values()) {
				if (codec.encodingName.equals(videoCodecName)) {
					...
					video.addAttributeField(
							new AttributeField(AttributeKey.rtpmap, RTPCodecEnum.getRTPMapString(selectedVideoCodec)));
					if (localSdp.isFirefox()) {
						...
						video.addAttributeField(
								new AttributeField(AttributeKey.extmap, "2 urn:ietf:params:rtp-hdrext:toffset"));
						video.addAttributeField(new AttributeField(AttributeKey.extmap,
								"3 http://www.webrtc.org/experiments/rtp-hdrext/abs-send-time"));
						video.addAttributeField(
								new AttributeField(AttributeKey.extmap, "4 urn:3gpp:video-orientation"));
					}
		...
		if (audio != null) {
			...
			for (RTPCodecEnum codec : RTPCodecEnum.values()) {
				if (codec.encodingName.equals(audioCodecName)) {
					...
					audio.addAttributeField(new AttributeField(AttributeKey.mid,
							offerSdp.getMediaDescription(SDPMediaType.audio).getMediaId()));
					if (localSdp.isFirefox()) {
						audio.addAttributeField(new AttributeField(AttributeKey.extmap,
								"1 urn:ietf:params:rtp-hdrext:ssrc-audio-level"));
					} else {
			...
			for (RTPCodecEnum codec : RTPCodecEnum.values()) {
				if (codec.encodingName.equals(videoCodecName)) {
					...
					video.addAttributeField(
							new AttributeField(AttributeKey.rtpmap, RTPCodecEnum.getRTPMapString(selectedVideoCodec)));
					// get media id from offer
					video.addAttributeField(new AttributeField(AttributeKey.mid,
							offerSdp.getMediaDescription(SDPMediaType.video).getMediaId()));
					video.addAttributeField(new AttributeField(AttributeKey.mid, "video"));
					video.addAttributeField(
							new AttributeField(AttributeKey.extmap, "2 urn:ietf:params:rtp-hdrext:toffset"));
					video.addAttributeField(new AttributeField(AttributeKey.extmap,
	...
	public void testAnswerChromeH264VideoOnly() throws SDPException {
		...
		assertEquals("126 level-asymmetry-allowed=1;packetization-mode=1;profile-level-id=42e01f",
				video.getAttribute(AttributeKey.fmtp).getValue());
	}
	...
	public void testGetBandwidth() throws SDPException {
		...
		if (bw != null) {
			...
			for (MediaField media : medias) {
				...
				if (bw != null) {
					System.out.printf("Bandwidth (%s) - type: %s value: %d%n", media.getMediaType(), bw.getType(),
							bw.getBandwidth());
				}
			}
		}
