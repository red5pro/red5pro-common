...
/**
 * A <tt>Format</tt> abstracts an exact media format. It carries no encoding-specific parameters or timing information global to the
 * presentation.
 * <br>
 * <h3>Comparing different formats</h3> Not all of the attributes in a <tt>Format</tt> object have to be specified. This enables selected attributes
 * to be specified, making it possible to locate a supported <tt>Format</tt> that meets certain requirements without needing to find an exact match.
 * <br>
 * Two methods are provided for comparing <tt>Formats</tt>. The <tt>equals</tt> method returns <tt>true</tt> if two <tt>Format</tt> objects are exactly the
 * same--they're the same type and all of their attributes are the same. The <tt>matches</tt> method relaxes the comparison, comparing only the attributes
