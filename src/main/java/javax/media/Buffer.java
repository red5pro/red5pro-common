...
/**
 * A <tt>Buffer</tt> is a media-data container that carries media data from one processing stage to the next inside of a <tt>Player</tt> or
 * <tt>Processor</tt>. <tt>Buffer</tt> objects are also used to carry data between a buffer stream and its handler.
 * <br>
 * A <tt>Buffer</tt> object maintains information such as the time stamp, length, and <tt>Format</tt> of the data it carries, as well as any header
...
public class Buffer {
    ...
    /**
     * Returns whether or not a given codec is compatable with the current format / codec decribing the data.
     * 
     * @param checkCodec
     * @return true if compatible and false otherwise
     */
    public boolean isCompatible(RTPCodecEnum checkCodec) {
        // if format/codec aren't set just return ok, this allows EOM/EOS to flow to the sink
        return codec.equals(checkCodec) || codec.equals(RTPCodecEnum.NONE);
    }

    /**
     * Gets the internal data object that holds the media chunk contained in this <tt>Buffer</tt>.
     *
    ...
    /**
     * Gets the header information for the media chunk contained in this <tt>Buffer</tt>.
     *
     * @return The object that holds the header information. Use <tt>instanceOf</tt> to determine what type the header object is
     * @see #header
     */
    ...
    /**
     * Gets the length of the valid data in this <tt>Buffer</tt> if the data is held in an array.
     *
     * @return The length of the valid data in the data array that holds the media chunk for this Buffer
     * @see #length
     */
     ...
     *
     * @return <tt>true</tt> if the KEY_FRAME flag is enabled, <tt>false</tt> if it is not.
     * @see #getFlags
     * @see #FLAG_KEY_FRAME
     */
     ...
     * 
     * @param keyframe true if this is a keyframe and false otherwise
     */
    ...
    /**
     * Sets the marker bit to signify the last packet is set. This is not to be confused with the end of message bit.
     * 
     * @param last true if this is the last packet and false otherwise
     */
    ...
    public String toString() {
        return String.format("Buffer %s [timeStamp=%d, sequenceNumber=%d, keyframe=%b, last=%b]", (codec.equals(RTPCodecEnum.NONE) ? "" : codec.name()), timeStamp, sequenceNumber, isKeyframe(), isLastPacket());
    }
