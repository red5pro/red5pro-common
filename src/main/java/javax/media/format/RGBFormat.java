...
/**
 * Describes uncompressed RGB data. The data is in interleaved form. RGB components can be packed into a short or an int. If the pixel components are
 * packed, then the <tt>redMask</tt>, <tt>greenMask</tt> and <tt>blueMask</tt> fields specify the bit masks, otherwise they specify the order of arrangement
 * of the components. For example:
 * <UL>
 * <LI>32-bit packed RGB stored as 32-bit integers would have the following masks: redMask = 0x00FF0000, greenMask = 0x0000FF00, blueMask =
 * 0x000000FF.</LI>
 * <LI>24-bit RGB stored as bytes would have the following masks: redMask = 1, greenMask = 2, blueMask = 3 and pixelStride = 3.</LI>
 * </UL>
 */
