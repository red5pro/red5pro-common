...
public class RTPCodecFactory {
	...
	private RTPCodecEnum[] availableVideoCodecs = {RTPCodecEnum.H264_PMODE1, RTPCodecEnum.H264_PMODE0,
			RTPCodecEnum.VP8};
	...
	// 2 - Interleaved Mode
	// All browsers *must support* mode 0 per
...

	{
		...
		audioAttributes.put(RTPCodecEnum.OPUS, new AttributeField(AttributeKey.fmtp,
				String.format("%d minptime=10; useinbandfec=1", RTPCodecEnum.OPUS.payloadType)));
		...
		// per spec: WebRTC implementations MUST signal this information in-band; as a
		// result, this parameter (sprop-parameter-sets) will not be present in SDP.
		videoAttributes.put(RTPCodecEnum.H264_PMODE0, new AttributeField(AttributeKey.fmtp, String
				.format("%d profile-level-id=%s;packetization-mode=0", RTPCodecEnum.H264_PMODE0.payloadType, profile)));
		videoAttributes.put(RTPCodecEnum.H264_PMODE1, new AttributeField(AttributeKey.fmtp, String
				.format("%d profile-level-id=%s;packetization-mode=1", RTPCodecEnum.H264_PMODE1.payloadType, profile)));
	}
	...
	public RTPCodec getAudioCodec(String codecName) {
		...
		for (RTPCodecEnum rc : availableAudioCodecs) {
			...
			if (encName.equals(codecName)) {
				String deviceClassName = String.format("com.red5pro.webrtc.media.device.%s.%sDevice",
						encName.toLowerCase(), encName.toUpperCase());
	...
	public RTPCodecEnum[] getAvailableAudioCodecsWithPrecedence(String codecsPrecedence) {
		...
		int codecsIndex = 0;
		log.debug("getAvailableAudioCodecsWithPrecedence - codecsPrecedence: {} finalIndex: {}", codecsPrecedence,
				finalIndex);
		while (initIndex < finalIndex) {
			...
			for (RTPCodecEnum rc : availableAudioCodecs) {
				if (rc.payloadType == Integer.valueOf(codecId)) {
					log.debug("getAvailableAudioCodecsWithPrecedence - codecId: {} codecName: {}", rc.payloadType,
							rc.encodingName);
			...
			log.debug("getAvailableAudioCodecsWithPrecedence - codecsIndex: {} initIndex: {} finalIndex: {}",
					codecsIndex, initIndex, finalIndex);
		}
