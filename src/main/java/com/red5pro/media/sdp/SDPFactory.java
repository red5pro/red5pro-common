...
public final class SDPFactory {
	 ...
	 * @return SessionDescription
	 */
	public static SessionDescription createSessionDescription(String userName, String address, int audioPort,
			int audioRTCPPort, int videoPort, int videoRTCPPort) {
		if (log.isDebugEnabled()) {
			log.debug("Create sdp - userName: {} address: {} audio port: {} video port: {}",
					new Object[]{userName, address, audioPort, videoPort});
		}
		SessionDescription sdp = new SessionDescription();
		// set the origin
		OriginField origin = new OriginField(userName, IdGenerator.generateNumericStringId(13), 1L, address);
		...
		if (audioPort != -1) {
			...
			if (audioRTCPPort == -1) {
				media.addAttributeField(new AttributeField(AttributeKey.rtcp, String.format("%d %s %s %s", audioPort,
						connection.getNetworkType(), connection.getAddressType(), address)));
				media.addAttributeField(new AttributeField(AttributeKey.rtcpmux, null));
				sdp.setRtcpMux(true);
			} else {
				media.addAttributeField(new AttributeField(AttributeKey.rtcp, String.format("%d %s %s %s",
						audioRTCPPort, connection.getNetworkType(), connection.getAddressType(), address)));
			}
			// add rtpmap
		...
		if (videoPort != -1) {
			...
			media.setConnection(connection);
			if (videoRTCPPort == -1) {
				media.addAttributeField(new AttributeField(AttributeKey.rtcp, String.format("%d %s %s %s", videoPort,
						connection.getNetworkType(), connection.getAddressType(), address)));
				media.addAttributeField(new AttributeField(AttributeKey.rtcpmux, null));
				sdp.setRtcpMux(true);
			} else {
				media.addAttributeField(new AttributeField(AttributeKey.rtcp, String.format("%d %s %s %s",
						videoRTCPPort, connection.getNetworkType(), connection.getAddressType(), address)));
			}
			// add rtpmap
			...
			for (RTPCodecEnum codec : codecs) {
				 ...
				 */
				if (fir) {
					media.addAttributeField(
							new AttributeField(AttributeKey.rtcpfb, String.format("%d ccm fir", codec.payloadType)));
				}
				// TODO have to fix negotiate on the feedback records below
				if (nack) {
					media.addAttributeField(
							new AttributeField(AttributeKey.rtcpfb, String.format("%d nack", codec.payloadType)));
				}
				if (pli) {
					media.addAttributeField(
							new AttributeField(AttributeKey.rtcpfb, String.format("%d nack pli", codec.payloadType)));
				}
				if (googRemb) {
					media.addAttributeField(
							new AttributeField(AttributeKey.rtcpfb, String.format("%d goog-remb", codec.payloadType)));
				}
				if (transportCC) {
					media.addAttributeField(new AttributeField(AttributeKey.rtcpfb,
							String.format("%d transport-cc", codec.payloadType)));
				}
			}
			...
			if (red) {
				media.addAttributeField(
						new AttributeField(AttributeKey.rtpmap, RTPCodecEnum.getRTPMapString(RTPCodecEnum.RED)));
			}
			if (ulpfec) {
				media.addAttributeField(
						new AttributeField(AttributeKey.rtpmap, RTPCodecEnum.getRTPMapString(RTPCodecEnum.ULPFEC)));
			}
			// add media to sdp
			sdp.addMediaDescription(media);
		}
	...
	public static SessionDescription senderCapabilitiesToSdp(JSONObject senderCaps) {
		...
		AttributeField finger = new AttributeField(AttributeKey.fingerprint,
				((JSONObject) fingerprints.get(0)).getAsString("algorithm") + ' '
						+ ((JSONObject) fingerprints.get(0)).getAsString("value"));
		...
		if (sendAudioCaps != null) {
			...
			for (int i = 0; i < acodecs.size(); i++) {
				...
				if ("opus".equals(acodec.getAsString("name"))) {
					...
					String rtpMapStr = String.format("%d opus/%d/%d", payloadType,
							(int) acodec.getAsNumber("clockRate"), (int) acodec.getAsNumber("numChannels"));
					media.addAttributeField(new AttributeField(AttributeKey.rtpmap, rtpMapStr));
					// set media attrs for opus
					media.addAttributeField(new AttributeField(AttributeKey.fmtp, String
							.format("%d minptime=%d; useinbandfec=1", payloadType, (int) acodec.getAsNumber("ptime"))));
		...
		if (sendVideoCaps != null) {
			...
			for (int i = 0; i < vcodecs.size(); i++) {
				...
				if ("H264".equals(vcodec.getAsString("name"))) {
					...
					media.addAttributeField(new AttributeField(AttributeKey.fmtp,
							String.format("%d profile-level-id=%s;packetization-mode=1", payloadType, profile)));
	...
	public static String receiverCapabilitiesFromSdp(SessionDescription sdp) {
		// log.debug("Incoming sdp: {}", sdp.toString(),
		// sdp.getMediaDescriptions().length);
		...
		String recvCaps = String.format(
				"{\"ice\":{\"usernameFragment\":\"%s\",\"password\":\"%s\"},\"dtls\":{\"role\":\"%s\",\"fingerprints\":[{\"algorithm\":\"sha-256\",\"value\":\"%s\"}]},"
	...
	public static ConnectionField createConnectionField(String connection) {
		...
		if (parts.length == 3) {
			ConnectionField field = new ConnectionField(parts[2], NetworkManager.NetworkType.valueOf(parts[0]),
					NetworkManager.AddressType.valueOf(parts[1]));
	...
	public static MediaField createMediaField(String media) {
		...
		if (log.isDebugEnabled()) {
			...
			log.debug("type: {} port: {} protocol: {} format(s): {}", matcher.group(1), matcher.group(2),
			...
			switch (type) {
				case application :
					...
					if (proto.contains("SCTP")) {
						...
						field = new MediaField(SDPMediaType.application, Integer.valueOf(matcher.group(2)), proto,
								new int[]{Integer.valueOf(matcher.group(4).trim())});
					}
					break;
				case audio :
				case video :
					...
					if (attr != null) {
						...
						if (mediaField == null) {
							...
							if (sdp.isRed5ProSDK()) {
								...
								SDPTrack track = SDPMediaType.audio.equals(mediaField.getMediaType())
										? sdp.getAudioTrack()
										: sdp.getVideoTrack();
								// if the attribute is control, create a track with the first payload id
								if (AttributeKey.control.equals(key)) {
									if (track == null) {
										track = new SDPTrack(sdp, mediaField.getMediaType(),
												mediaField.getFormats()[0]);
									}
									track.setControl(attr.getValue());
									log.debug("Created new track for control: {}", track);
								} else if (AttributeKey.fmtp.equals(key)) {
									// if the user agent is red5 pro sdk get the track and apply these incoming
									// parameters
									// if the track is null, control doesnt exist in the parse yet or fmtp came
									// before it
									if (track == null) {
										track = new SDPTrack(sdp, mediaField.getMediaType(),
												mediaField.getFormats()[0]);
									...
									// sprop-parameter-sets=Z0LAINkAoD2hAAADAAEAAAMAMA8YMkg=,aMuDyyA=;
									// profile-level-id=42C020
									// a=fmtp:97
									...
									for (String fmtParam : fmtParams) {
										if (StringUtils.isNotBlank(fmtParam)) {
											if (fmtParam.contains("=")) {
												...
												if (fmtParam
														.startsWith(VideoConstants.H264_SPROP_PARAMETER_SETS_FMTP)) {
													track.addParameter(VideoConstants.H264_SPROP_PARAMETER_SETS_FMTP,
															fmtParam.substring(fmtParam.indexOf('=') + 1));
												} else {
