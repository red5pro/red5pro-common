...
public class SDPTrack {
	...
	public void setControl(String control) {
		...
		// aggregate control will be represented by an integer, otherwise we'll get an
		// rtsp uri
		if (control.contains("rtsp://")) {
			...
			if (NumberUtils.isCreatable(control)) {
				...
				if (controlAttr != null) {
					...
					mediaField.addAttributeField(SDPFactory.createAttributeField(AttributeKey.control,
							String.format("trackID=%d", controlChannelId)));
				}
	...
	public int getControlChannelId() {
		if (control != null) {
			// AAC and H264 class look for "streamid"; so hopefully we catch all these
			// variants using the first split item
			// a=control:trackId=1 (seems that mixed case is possible)
	...
	public String toString() {
		return "SDPTrack [mediaType=" + mediaType + ", payloadId=" + payloadId + ", control=" + control
				+ ", mediaChannelId=" + mediaChannelId + ", interleavedVal=" + interleavedVal + ", parameters="
				+ parameters + "]";
	}
