 ...
 * @author Nate Roe
 */
public class LiveInterstitial extends InterstitialSession
		implements
			IStreamListener,
			ProStreamTerminationEventListener {
	 ...
	 *            send audio?
	 */
	public LiveInterstitial(IInterstitialStream liveStream, IInterstitialStream newStream, boolean isInterstitialAudio,
			boolean isInterstitialVideo) {
		super(isInterstitialAudio, isInterstitialVideo);

		log.trace("LiveInterstitial ctor. isForwardVideo: {}, isForwardAudio: {}", isInterstitialVideo,
				isInterstitialAudio);
	...
	public void packetReceived(IBroadcastStream stream, IStreamPacket packet) {
		...
		if (packet instanceof VideoData) {
			if (!isVideoPrimed) {// send codec configs if present.
				isVideoPrimed = true;
				if (newStream.getCodecInfo().getVideoCodec() != null
						&& newStream.getCodecInfo().getVideoCodec().getDecoderConfiguration() != null) {
					VideoData privateConfig = new VideoData(
							newStream.getCodecInfo().getVideoCodec().getDecoderConfiguration());
			...
			if (newStream.getCodecInfo().getAudioCodec() != null
					&& newStream.getCodecInfo().getAudioCodec().getDecoderConfiguration() != null) {
				AudioData privateConfig = new AudioData(
						newStream.getCodecInfo().getAudioCodec().getDecoderConfiguration());
				privateConfig.setTimestamp(packet.getTimestamp() - timeDelta);
				liveStream.dispatchInterstitial(privateConfig);
			} // else forthcoming
